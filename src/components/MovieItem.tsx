...
const MovieItem: FC<IMovieDataSingle> = ({ movie }) => {
    return (
        <div>
            ...
            <Link href={`film/${movie?.title}/${movie?.id}`} className="flex items-center w-100 h-72 m-5 p-5 space-x-4 bg-yellow-100 rounded-3xl shadow-xl hover:brightness-75 duration-300 max-mini:w-64 max-mini:h-min max-mini:flex-col">
                <img className='h-64 w-170 rounded-3xl' src={movie?.medium_cover_image} alt={movie?.title} />
                <div className="flex flex-col h-64 justify-between max-mini:h-min max-mini:w-72 max-mini:items-center max-mini:!ml-0">
                    <div className="h-40 w-36 overflow-hidden max-mini:h-min max-mini:mb-3">
                        <p className="max-mini:text-center">{movie?.title}</p>
                        <p className="text-xl max-mini:flex max-mini:justify-center">{movie?.year}</p>
                    </div>
                    <div className="flex items-center">
                        ...
                        <div className="text-lg flex flex-wrap justify-center items-center max-mini:flex-col">
                        <div>Rating:</div>
