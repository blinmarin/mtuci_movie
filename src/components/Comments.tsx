...
const Comments: FC<IFilmData> = ({ movie }) => {
    ...
    return (
        <div>
            <div className="flex flex-col w-650 px-20 pt-5">
                <div className="mb-5">Leave a comment</div>

                <div className="flex">
                    <p className="mr-5">Your name:</p>
                    <input className='w-60 h-10 text-amber-800 bg-amber-25 border-4 border-amber-800 rounded-xl p-2 text-lg focus:border-amber-300 outline-none mb-5' type='text' value={comentators} onChange={event => setComentators(event.target.value)}></input>
                </div>
                <textarea className='w-500 h-24 text-amber-800 bg-amber-25 border-4 border-amber-800 rounded-xl p-2 text-lg focus:border-amber-300 outline-none mb-5' value={comments} onChange={event => setComments(event.target.value)}></textarea>
                <div className="flex flex-row-reverse">
                    <button className="w-60 h-10 bg-amber-800 text-amber-100 rounded-xl p-1 text-xl font-medium  hover:bg-amber-300 hover:text-amber-800 hover:scale-110 hover:transition hover:duration-500" onClick={writeComment}>Leave a comment</button>
                </div>
            </div>
            ...
            <div className="flex flex-col items-center  m-10">
                ...
                {storageData ? storageData.map((data) => <CommentItem data={data} />) : <div className="mb-10">There are no comments yet. Become the first!</div>}
            </div>
